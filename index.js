...
program
  ...
  .description('CLI utility to convert TON wallet addresses to different formats')
  .version('1.0.0');

program
  .argument('<address>', 'TON wallet address to convert')
  .option('-f, --format <format>', 'specific format to output (raw, user-friendly, bounceable, non-bounceable)', 'all')
  ...
  .action((addressInput, options) => {
    try {
      ...
      if (options.format === 'all') {
        ...
        results.bounceableTestnet = address.toString({ bounceable: true, testOnly: true });
        results.nonBounceableTestnet = address.toString({ bounceable: false, testOnly: true });
      ...
      const transformCase = (str, formatKey) => {
        if (formatKey === 'raw') {
          ...
          if (parts.length === 2) {
            ...
            let hexPart = parts[1];
            if (options.uppercase) hexPart = hexPart.toUpperCase();
            if (options.lowercase) hexPart = hexPart.toLowerCase();
            return `${workchain}:${hexPart}`;
          }
          // Fallback if raw string is not in "workchain:hex" format, apply to whole string
        }
        // For other formats or fallback for raw
        if (options.uppercase) return str.toUpperCase();
        if (options.lowercase) return str.toLowerCase();
      ...
      for (const [formatName, value] of Object.entries(results)) {
        const displayName = formatName
          ...
          .trim();
        console.log(`${displayName.padEnd(25)}: ${transformCase(value, formatName)}`);
      }
